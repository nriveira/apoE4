function pf = get_circtrack_pfs_backwardExpansion_v2(rateMap, varargin)
% function pf = get_circtrack_pfs_backwardExpansion_v2(rateMap, minMeanFr, minPkFr, trackDiameter)
% 
% PURPOSE:
...
[pks, pkLocs] = findpeaks(rateMap, 'MinPeakHeight', minPkFr);

inds = [];
boundFr = 0.1 * max(rateMap); %firing rate threshold for bounds
for i = 1:length(pks)
    startInd = find(rateMap(1:pkLocs(i))<boundFr, 1, 'Last');
    if isempty(startInd)
        startInd = 0;
    end 
    endInd = find(rateMap(pkLocs(i):end)<boundFr, 1, 'First');
    endInd = endInd + pkLocs(i) - 1;
