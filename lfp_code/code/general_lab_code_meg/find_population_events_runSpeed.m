function popEvents = find_population_events_runSpeed(allSpkTmsbyCell, runSpeed, varargin)
% 
%   NOTE:
%       Same as find_population_events, just uses runspeed as input instead
%       of coords.
%
%   PURPOSE:
...
%
%   INPUT:
%       runSpeed = run speed formatted as:
%           (:,1) = times in seconds
%           (:,2) = speed in cm/s
...
if speedOpt == 1
    ...
    for u = 1:length(allSpkTmsbyCell)
        tmpSpkTms = allSpkTmsbyCell{u};
        
        for st = 1:length(tmpSpkTms)
            rsInd = match(tmpSpkTms(st), runSpeed(:,1));
            if runSpeed(rsInd,2) < runThresh
...
startTm = runSpeed(1,1);
endTm = runSpeed(end,1);
...
for i = 1:size(potEvents,1)
    ...
    for u = 1:length(usedSpkTmsbyCell) %for each unit
        tmpSpkTms = usedSpkTmsbyCell{u}; %pull out the spike times
