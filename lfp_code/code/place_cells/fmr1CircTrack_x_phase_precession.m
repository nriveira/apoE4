...
spkInfo = cell(1,1); %for down sampling spikes

ppSlopes = cell(1,1); %phase precession slopes by group
ppSlopesCm = cell(1,1);
ppPhaseOffset = cell(1,1);
ppPhaseRange = cell(1,1); %phase range = phase difference between the
% spike with the highest shifted phase and the spike with the lowest
% shifted phase (shifted phase is minus phase offset)
ppPhaseRangeAlt = cell(1,1); %alt phase range - pp slope * pf size
ppR2 = cell(1,1); %r2 values from circular regression

cols = {'Blue'};
groupNames = {'apoE4'};

saveDir = 'E:\Rat381\results\phasePrecession';
curDir = pwd;
...
figtitle = 'PhasePrecession_Slope_cm';
figure('Name', figtitle, 'Position', [425 436 781 420])
