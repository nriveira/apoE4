function adprojCircTrack_x_phase_precession_lapByLap(group)
...
%
% OUTPUT:
%   F(n-n)(optional): Plot for each unit comparing lap to overall average from the
%       first 10 (or selected number of laps).
%   F1: Slope of the phase precession using two unit measurements:
%       - deg/place field
%       - deg/cm on the track
%   F2: Phase offset across laps.
%   F3: Phase range using 2 methods
%       - Phase difference between the spike with the highest shifted phase
%       and the spike with the lowest shifted phase (Feng et al. 2015).
%       - Multiplication of phase precession slope and the size of the
%       single-lap place field (Schmidt et al. 2009).
%   F4: R2 value of phase precession using circular-linear regression
%       across laps.
%
% OPTIONS:
...
saveDir = 'E:\resultsFeb2023_AD_WT\phasePrecession\byLap'; %for saving the pahse precession figs
...
% lapSlopes = cell(1,numLaps); %group x first 10 laps individually
% bSlopes = cell(1,1); %all together
% cmLapSlopes = cell(1,numLaps);
% cmbSlopes = cell(1,1);
% lapPhaseOff = cell(1,numLaps);
% bPhaseOff = cell(1,1);
% lapPhaseRange = cell(1,numLaps);
% bPhaseRange = cell(1,1);
% altLapPhaseRange = cell(1,numLaps);
% altBPhaseRange = cell(1,1);
% lapR2 = cell(1,numLaps);
% bR2 = cell(1,1);
                        ...
                        if stableCheck == 1
                            ...
                            for b = 1:4
                                ...
                                for ps = 1:size(pfPasses{b},1)
                                    ...
                                    rsStartInd = match(pfPasses{b}(ps,1), runSpd(:,1));
                                    rsEndInd = match(pfPasses{b}(ps,2), runSpd(:,1));
                                    
                                    if min(runSpd(rsStartInd:rsEndInd,2)) < psSpdThresh
                                        continue %to next pass
                                    end %ccheck speed through the pass
                            ...
                            [beta,R2,p] = CircularRegression(allSpkPos,deg2rad(allSpkPhis)); %same answer as using bound = 2
