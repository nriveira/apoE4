function adprojCircTrack_x_plot_spikeTimeAutocorrelations(group)
...
plotByDay = 1; %1 to plot mean for every day, 0 to not
plotByRat = 1; %1 to plot mean for every rat, 0 to not
...
for g = 1:2
    ...
    for r = 1:length(group(g).rat)
        ...
        if plotByRat == 1
            ...
            plotData = mean(autoCorrByRat,1,'omitnan');
            plotData(ceil(length(plotData)/2)) = NaN;
            plot(linspace(-timeLag, timeLag, length(plotData)), plotData, 'Color', rgb(cols{g}), 'LineWidth', 1.5)
...
for g = 1:2
    ...
    plotData = mean(autoCorr{g},1,'omitnan');
    plotData(ceil(length(plotData)/2)) = NaN;
    plot(linspace(-timeLag, timeLag, length(plotData)), plotData, 'Color', rgb(cols{g}), 'LineWidth', 1.5)
